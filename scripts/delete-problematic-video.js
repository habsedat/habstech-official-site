 ...
 * Delete Problematic Video Script - Remove video with blob URL
 */
...
async function deleteProblematicVideo() {
  console.log('🗑️ Starting problematic video deletion...');
  
  try {
    // Find the video with blob URL
    ...
    for (const docSnapshot of mediaSnapshot.docs) {
      ...
      if (data.name === 'H E Intro.mp4' && data.thumbnail && data.thumbnail.startsWith('blob:')) {
        console.log(`   ❌ Found problematic video with blob URL`);
        ...
        if (data.firebaseUrl) {
          try {
            const videoRef = ref(storage, data.firebasePath || `media/videos/${data.filename}`);
        ...
        try {
          ...
          console.log(`\n🎉 Problematic video deleted successfully!`);
          console.log(`   You can now re-upload the video and it will have proper thumbnails.`);
          return;
        } catch (error) {
    ...
    console.log(`\nℹ️ No problematic video found to delete`);
    
  } catch (error) {
    console.error('❌ Error deleting problematic video:', error);
  }
}

// Run the deletion
deleteProblematicVideo();
